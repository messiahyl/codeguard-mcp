...
export function scanCode(code, language, options) {
    const lines = code.split("\n");
    const rules = getAllRules().filter((rule) => {
        if (options.rulesFilter && options.rulesFilter.length > 0) {
            return options.rulesFilter.some((f) => rule.id.toUpperCase().includes(f.toUpperCase()) || f.toUpperCase() === rule.category.toUpperCase());
        }
        return true;
    });
    const results = [];
